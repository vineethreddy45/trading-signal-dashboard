...
def scan_symbols(symbols_df: pd.DataFrame, timeframe: str, limit: int | None = None) -> pd.DataFrame:
    ...
    for item in source.itertuples(index=False):
        try:
            ...
            rows.append({"Symbol": item.display_symbol, "Yahoo Symbol": item.symbol, "Market": item.market,
                         "Timeframe": timeframe, "Signal": signal["signal"], "Close": signal["close"],
            ...
            rows.append({"Symbol": item.display_symbol, "Yahoo Symbol": item.symbol, "Market": item.market,
                         "Timeframe": timeframe, "Signal": "ERROR", "Error": str(exc)})
